...
export default function ProcedureForm() {
  const [state, formAction, pending] = useActionState(addProcedure, null);
  ...
  return (
    <form ref={formRef} action={formAction} className="flex flex-col gap-3 rounded-xl border border-border bg-surface p-5">
      <h2 className="text-sm font-semibold text-ink">إضافة إجراء جديد</h2>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <input name="name" type="text" placeholder="اسم الإجراء" required className={inputClass} />
        <input name="category" type="text" placeholder="التصنيف (اختياري)" className={inputClass} />
        <input name="base_price" type="number" step="0.01" placeholder="السعر الأساسي" className={inputClass} />
      </div>
