...
export default function PerioForm({ patients }: { patients: Patient[] }) {
  const [state, formAction, pending] = useActionState(addPerioEntry, null);
  ...
  return (
    <form ref={formRef} action={formAction} className="flex flex-col gap-3 rounded-xl border border-border bg-surface p-5">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-5">
        ...
        <input name="pocket_depth" type="number" step="0.5" placeholder="عمق الجيب (مم)" className={inputClass} />
