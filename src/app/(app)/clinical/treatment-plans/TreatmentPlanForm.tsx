...
export default function TreatmentPlanForm({ patients }: { patients: Patient[] }) {
  const [state, formAction, pending] = useActionState(addTreatmentPlan, null);

  return (
    <form action={formAction} className="flex flex-col gap-3 rounded-xl border border-border bg-surface p-5">
      <h2 className="text-sm font-semibold text-ink">إنشاء خطة علاج جديدة</h2>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <input name="title" type="text" placeholder="عنوان الخطة" required className={inputClass} />
